...
func main() {
	...
	commando.
		Register("getEndpoint").
		SetShortDescription("displays detailed information of a endpoint").
		SetDescription("This command search's if an endpoint is exposed.").
		AddArgument("path", "search for a endpoint", "/v1/accounts").
		AddFlag("env", "search for a specific environment", commando.String, "all"). // default `all`
		SetAction(func(args map[string]commando.ArgValue, flags map[string]commando.FlagValue) {
			dir := args["dir"].Value
